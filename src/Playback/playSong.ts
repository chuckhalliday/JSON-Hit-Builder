...
export async function playDrums(bpm: number, midi: boolean, beat: number, partDrumGroove: number[], partDrums: DrumHit[][], onStep: (lampIndex: number) => void, shouldStop?: () => boolean): Promise<number> {
  const [drumEnd] = await Promise.all([
    playBeat(midi, beat, partDrums[0], partDrumGroove, bpm, partDrums, onStep, shouldStop),
